...
parser.add_argument('--predictor', type=str, default= "forest", help='Specify the type of the predictor, should be one of "forest", "xgboos')
parser.add_argument('--n_tolerant_rounds', type=int, default= 2, help='Specify the number of tolerant rounds when handling early stopping,') 
parser.add_argument('--drug_encoder', type=str, default= 'molebert', help='The encoder of the drug sequence you want to use')
parser.add_argument('--feature_fusion', type=str, default= 'cross_attention', help='How to concatenate two drug vector')
parser.add_argument('--classifier', type=str, default= 'deepforest', help='Choose the predict model')
...
def mol_to_graph_data_obj_simple(mol):
    ...
    if len(mol.GetBonds()) > 0:# mol has bonds
        ...
        edge_attr = torch.tensor(np.array(edge_features_list),dtype=torch.long) # Edge feature matrix with shape [num_edges, num_edge_featu
    else:  # no bond
...
def gnnsmile(s):
    ...
    model = GNN_graphpred(5, 300, 1,"last", 0.5, graph_pooling = 'mean', gnn_type = 'gin')# 5 GNN layers, 300 dimensionality of embeddings,
...
for i in ftrain:
    try:
        ...
        try:
            ...
            if args.drug_encoder == 'molebert':
                drug_fingerprints = gnnsmile(a)#getting the numerical representation of the drug smiles sequence, dimensionality for drug v
...
print(len(removelisttrain))
print(len(removelisttest))
print('dp done')

# Attention to drug vector pair
device = torch.device("cuda:" + "0") if torch.cuda.is_available() else torch.device("cpu")
for i in range(len(train)):
    if args.feature_fusion == 'cross_attention':
        try:
            ...
            X_train.append(alldrugtrain[train.iloc[i,0]]+alldrugtrain[train.iloc[i,2]]+result_attention1.tolist()[0] + result_attention2.tolist()[0])
            #Final Vector Dimension is 700
            if train.iloc[i,4] == args.prediction_type:# getting the label for positive or negative samples
                Y_train.append(1)
            else:
                Y_train.append(0)
        except:
            removelisttrain.append(i)
            continue
...
for i in range(len(test)):
    if args.feature_fusion == 'cross_attention':
        try:
            ...
            if test.iloc[i,4] == args.prediction_type:
                Y_test.append(1)
            else:
                Y_test.append(0)
        except:
            removelisttest.append(i)
            continue
    else:
        try:
...
removelisttrain = list(set(removelisttrain))
removelisttest = list(set(removelisttest))
print(len(removelisttrain))
train = train.drop(removelisttrain)
test = test.drop(removelisttest)


print(len(train))
print(len(test))

train.to_csv('./data/modeltrainingdata/trainfor49.csv')
test.to_csv('./data/modeltrainingdata/testfor49.csv')
